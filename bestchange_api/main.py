...
async def creation_date(path_to_file):
    """
...
class Rates:
    ...
    async def filter(self, give_id, get_id):
        data = []
...
class Common:
    ...
    async def get_by_id(self, id, only_name=True):
...
class Exchangers(Common):
    ...
    async def extract_reviews(self, rates):
        ...
        try:
            if os.path.isfile(self.__cache_path) \
                ...
                filename, headers = urlretrieve(
                    self.__url, self.__cache_path if self.__cache else None)
...
if __name__ == '__main__':
    ...
    api = BestChange(cache_seconds=1, exchangers_reviews=True,
                     split_reviews=True, ssl=True, proxy=proxy)
    print(api.is_error())
    ...
    for val in top:
        print(currencies[val['give_id']]['name'], '->',
              currencies[val['get_id']]['name'], ':', round(val['perc'], 2))
    ...
    print(title.format(dir_from, dir_to, api.currencies().get_by_id(
        dir_from), api.currencies().get_by_id(dir_to)))
    for val in rows[:3]:
